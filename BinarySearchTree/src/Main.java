...
//		tree.preOrder();
	...
	Node deleteRecursively(Node root, int key) {
		...
		// found the key
			...
			root.value = smallestValueInfo(root.right);
	...
	char smallestValueInfo(Node root) {
		char val = root.value;
		while (root.left != null) {
			val = root.left.value;
			root = root.left;
